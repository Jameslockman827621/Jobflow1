...
export default function AnalyticsPage() {
  ...
  // Map the real API response — the backend returns nested objects
  const realStats = {
    totalApplications: overview?.applications?.total_applications ?? overview?.total_applications ?? 0,
    interviewRate: overview?.interviews?.interview_rate ?? overview?.interview_rate ?? 0,
    offerRate: overview?.success?.offer_rate ?? overview?.offer_rate ?? 0,
    responseRate: overview?.success?.response_rate ?? overview?.response_rate ?? 0,
    profileCompleteness: overview?.profile_completeness ?? 0,
  };

  // Build funnel from real by_stage data if available
  const byStage = overview?.applications?.by_stage ?? overview?.by_stage ?? {};
  const totalApps = realStats.totalApplications;
  const funnel = totalApps > 0 ? [
    { stage: "Applied", count: totalApps, percentage: 100 },
    { stage: "Phone Screen", count: byStage.get?.("phone_screen") ?? byStage.phone_screen ?? 0, percentage: totalApps > 0 ? Math.round(((byStage.get?.("phone_screen") ?? byStage.phone_screen ?? 0) / totalApps) * 100) : 0 },
    { stage: "Technical", count: byStage.get?.("technical") ?? byStage.technical ?? 0, percentage: totalApps > 0 ? Math.round(((byStage.get?.("technical") ?? byStage.technical ?? 0) / totalApps) * 100) : 0 },
    { stage: "Onsite", count: byStage.get?.("onsite") ?? byStage.onsite ?? 0, percentage: totalApps > 0 ? Math.round(((byStage.get?.("onsite") ?? byStage.onsite ?? 0) / totalApps) * 100) : 0 },
    { stage: "Offer", count: byStage.get?.("offer") ?? byStage.offer ?? 0, percentage: totalApps > 0 ? Math.round(((byStage.get?.("offer") ?? byStage.offer ?? 0) / totalApps) * 100) : 0 },
  ] : [];

  // Use real market insights if available, otherwise show honest empty state
  const companies = marketInsights?.companies ?? [];
  const skills = marketInsights?.trending_skills ?? [];

  // Empty state for new users — return early to avoid complex JSX nesting
  if (realStats.totalApplications === 0) {
    return (
      <AppShell>
        <div className="mb-8">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-sm text-slate-500 mb-6 max-w-sm mx-auto">Apply to your first job to see analytics here — interview rates, response times, and your application funnel.</p>
          <a href="/dashboard" className="inline-block px-6 py-2.5 bg-teal-500 text-white rounded-lg font-semibold text-sm hover:bg-teal-600 transition-colors">
            Browse matched jobs →
          </a>
        </div>
      </AppShell>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="text-right">
            <div className="text-2xl font-bold text-slate-900">{realStats.profileCompleteness}%</div>
          </div>
        </div>
        <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
          <div className="h-full bg-teal-500 rounded-full transition-all" style={{ width: `${realStats.profileCompleteness}%` }}></div>
        </div>
